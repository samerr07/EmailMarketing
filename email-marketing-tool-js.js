...
function startSendingEmails() {
  ...
  const eventSource = new EventSource('/send?' + new URLSearchParams({
  ...
  eventSource.onmessage = function(event) {
    ...
    switch (data.type) {
      ...
      case 'progress':
        ...
        // Update counters
        elements.sentCount.textContent = success.toString();
        elements.failedCount.textContent = failed.toString();
        elements.remainingCount.textContent = (total - current).toString();
        
        // Log message
        if (data.status === 'success') {
          logMessage(`Email sent to ${data.email}`, 'success');
        } else {
          logMessage(`Failed to send to ${data.email}: ${data.error}`, 'error');
        }
        break;
      ...
      case 'complete':
        ...
        elements.progressCount.textContent = `${data.total} / ${data.total}`;
        elements.progressPercent.textContent = '100%';
        
        elements.sentCount.textContent = data.success.toString();
        elements.failedCount.textContent = data.failed.toString();
        elements.remainingCount.textContent = '0';
        
        // Show completion section
        elements.sendingComplete.classList.remove('hidden');
        elements.finalSentCount.textContent = data.success.toString();
        elements.finalFailedCount.textContent = data.failed.toString();
        elements.finalTotalCount.textContent = data.total.toString();
        
        // Log completion
        logMessage(`Sending complete: ${data.success} sent, ${data.failed} failed`, 'info');
